...
from model.data.bounding_box import BoxList
...
class VisualGenomeDataset(Dataset):
    def __init__(self, data_dir, split, transforms=None, num_im=-1, num_val_im=5000,
        ...
        self.info = json.load(
            open(os.path.join(self.data_dir, "VG-SGG-dicts.json"), 'r'))
...
def load_graphs(graphs_file, images_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    for i in range(len(image_index)):
        ...
        if filter_non_overlap:
            assert mode == 'train'
            # 重叠占比联合框面积的比例
            inters = bbox_overlaps(torch.from_numpy(
                boxes_i).float(), torch.from_numpy(boxes_i).float()).numpy()
