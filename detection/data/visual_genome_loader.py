...
class visual_genome(data.Dataset):
    def __init__(self, set_option, image_set, annotation_folder='top_150_50'):
        ...
        annotation_dir = osp.join(
            cfg.DATA_DIR, 'visual_genome', annotation_folder)
        cats = json.load(open(osp.join(annotation_dir, 'categories.json')))
        dictionary = json.load(open(osp.join(annotation_dir, 'dict.json')))
        inverse_weight = json.load(
            open(osp.join(annotation_dir, 'inverse_weight.json')))
        ...
        self._object_class_to_ind = dict(
            zip(self.object_classes, xrange(self.num_object_classes)))
        self._predicate_class_to_ind = dict(
            zip(self.predicate_classes, xrange(self.num_predicate_classes)))
        ...
        self.inverse_weight_object = self.inverse_weight_object / \
            self.inverse_weight_object.min()
        ...
        self.inverse_weight_predicate = self.inverse_weight_predicate / \
            self.inverse_weight_predicate.min()
    ...
    def __getitem__(self, index):
        ...
        target_scale = cfg.TRAIN.SCALES[npr.randint(
            0, high=len(cfg.TRAIN.SCALES))]
        img = cv2.imread(
            osp.join(cfg.IMG_DATA_DIR, self.annotations[index]['path']))
        img, im_scale = self._image_resize(
            img, target_scale, cfg.TRAIN.MAX_SIZE)
        im_info = np.array([img.shape[0], img.shape[1],
        ...
        # 4 for box and 40 for sentences
        gt_boxes_region = torch.zeros(
            (len(_annotation['regions']), self.max_size + 4))
        gt_boxes_object[:, 0:4] = torch.FloatTensor(
            [obj['box'] for obj in _annotation['objects']]) * im_scale
        gt_boxes_region[:, 0:4] = torch.FloatTensor(
            [reg['box'] for reg in _annotation['regions']]) * im_scale
        gt_boxes_object[:, 4] = torch.FloatTensor(
        ...
        gt_relationships = torch.zeros(len(_annotation['objects']), (len(
            _annotation['objects']))).type(torch.LongTensor)
    ...
    def get_regions(self, idx, length_constraint=50):
        boxes = np.array(
            [reg['box'] for reg in self.annotations[idx]['regions']]).astype(np.float32)
        text = []
