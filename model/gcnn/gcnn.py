...
from model.gcnn.agcn.agcn import _GraphConvolutionLayer_Collect, _GraphConvolutionLayer_Update

# to do
# make_roi_relation_feature_extractor, make_roi_relation_box_feature_extractor
# make_roi_relation_box_predictor, make_roi_relation_predictor
...
class GRCNN(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.pred_feature_extractor = make_roi_relation_feature_extractor(
            cfg, in_channels)
        ...
        if self.update_step > 0:
            self.gcn_collect_feat = _GraphConvolutionLayer_Collect(
                self.dim, self.dim)
            self.gcn_update_feat = _GraphConvolutionLayer_Update(
                self.dim, self.dim)
    ...
    def _get_map_idxs(self, proposals, proposal_pairs):
        ...
        for proposal, proposal_pair in zip(proposals, proposal_pairs):
            ...
            obj_obj_map[offset:offset +
                        len(proposal), offset:offset + len(proposal)] = obj_obj_map_i
            rel_ind_i += offset
            offset += len(proposal)
        ...
        subj_pred_map = rel_inds.new(
            obj_num, rel_inds.shape[0]).fill_(0).float().detach()
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        x_obj = torch.cat([proposal.get_field("features").detach()
        ...
        x_pred, _ = self.pred_feature_extractor(
            features, proposals, proposal_pairs)
        ...
        for t in range(self.update_step):
            ...
            obj_feats.append(self.gcn_update_feat(
                obj_feats[t], source2obj_all, 0))

            '''update predicate logits'''
            source_obj_sub = self.gcn_collect_feat(
                pred_feats[t], obj_feats[t], subj_pred_map.t(), 2)
            source_obj_obj = self.gcn_collect_feat(
                pred_feats[t], obj_feats[t], obj_pred_map.t(), 3)
            source2rel_all = (source_obj_sub + source_obj_obj) / 2
            pred_feats.append(self.gcn_update_feat(
                pred_feats[t], source2rel_all, 1))
        ...
        for t in range(self.update_step):
            ...
            # message from other objects
            source_obj = self.gcn_collect_score(
                obj_scores[t], obj_scores[t], obj_obj_map, 4)

            # essage from predicate
            source_rel_sub = self.gcn_collect_score(
                obj_scores[t], pred_scores[t], subj_pred_map, 0)
            source_rel_obj = self.gcn_collect_score(
                obj_scores[t], pred_scores[t], obj_pred_map, 1)
            source2obj_all = (source_obj + source_rel_sub + source_rel_obj) / 3
            obj_scores.append(self.gcn_update_score(
                obj_scores[t], source2obj_all, 0))

            '''update predicate logits'''
            source_obj_sub = self.gcn_collect_score(
                pred_scores[t], obj_scores[t], subj_pred_map.t(), 2)
            source_obj_obj = self.gcn_collect_score(
                pred_scores[t], obj_scores[t], obj_pred_map.t(), 3)
            source2rel_all = (source_obj_sub + source_obj_obj) / 2
            pred_scores.append(self.gcn_update_score(
                pred_scores[t], source2rel_all, 1))

        obj_class_logits = obj_scores[-1]
        pred_class_logits = pred_scores[-1]

        if obj_class_logits is None:
            logits = torch.cat([proposal.get_field("logits")
                                for proposal in proposals], 0)
